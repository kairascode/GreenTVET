...
@section('content')
    ...
    <form action="{{ route('allocations.update', $allocation) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="institution_id" id="institution_id" class="form-control @error('institution_id') is-invalid @enderror" required>
                ...
                @foreach($institutions as $institution)
                    <option value="{{ $institution->id }}" {{ $allocation->institution_id == $institution->id ? 'selected' : '' }}>{{ $institution->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="tree_species_id" id="tree_species_id" class="form-control @error('tree_species_id') is-invalid @enderror" required>
                ...
                @foreach($species as $specie)
                    <option value="{{ $specie->id }}" {{ $allocation->tree_species_id == $specie->id ? 'selected' : '' }}>{{ $specie->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="number" name="quantity_allocated" id="quantity_allocated" class="form-control @error('quantity_allocated') is-invalid @enderror" value="{{ old('quantity_allocated', $allocation->quantity_allocated) }}" required>
